...
const typeStyles: Record<ToastType, { bg: string; icon: string; border: string }> = {
  success: { bg: "bg-green-50 dark:bg-green-950", icon: "✓", border: "border-green-400 dark:border-green-700" },
  error:   { bg: "bg-red-50 dark:bg-red-950",     icon: "✕", border: "border-red-400 dark:border-red-700" },
  info:    { bg: "bg-blue-50 dark:bg-blue-950",   icon: "ℹ", border: "border-blue-400 dark:border-blue-700" },
  warning: { bg: "bg-amber-50 dark:bg-amber-950", icon: "⚠", border: "border-amber-400 dark:border-amber-700" },
};
...
export function ToastItem({ toast, onDismiss }: ToastItemProps) {
  ...
  return (
    ...
    >
      <span className="mt-0.5 text-lg" aria-hidden="true">{styles.icon}</span>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{toast.message}</p>
        {toast.description && (
...
export function useToast() {
  ...
  const addToast = useCallback(
    (partial: Omit<ToastMessage, "id">): string => {
