...
import { validateTransactionForm, type FieldError } from "@/lib/transactionValidation";
...
export function TransactionForm({
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-6 ${className}`} noValidate aria-label="Transaction form">
      <div>
        <label htmlFor="transaction-amount" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Amount</label>
        <div className="mt-1">
          <input
            id="transaction-amount" type="text" inputMode="decimal" value={amount}
            onChange={(e) => setAmount(e.target.value)} onBlur={() => handleBlur("amount")}
            disabled={isSubmitting}
            aria-invalid={touched.has("amount") && !!amountError}
            aria-describedby={amountError && touched.has("amount") ? "transaction-amount-error" : undefined}
            className={[
        ...
        {touched.has("amount") && amountError && (
          <p id="transaction-amount-error" role="alert" className="mt-1 text-sm text-red-600 dark:text-red-400">{amountError}</p>
        )}
      </div>
      <div>
        <label htmlFor="transaction-address" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Recipient Address</label>
        <div className="mt-1">
          <input
            id="transaction-address" type="text" value={address}
            onChange={(e) => setAddress(e.target.value)} onBlur={() => handleBlur("address")}
            disabled={isSubmitting}
            aria-invalid={touched.has("address") && !!addressError}
            aria-describedby={addressError && touched.has("address") ? "transaction-address-error" : undefined}
            className={[
              "block w-full rounded-lg border px-3 py-2 text-sm shadow-sm transition-colors",
              "focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50",
        ...
        {touched.has("address") && addressError && (
          <p id="transaction-address-error" role="alert" className="mt-1 text-sm text-red-600 dark:text-red-400">{addressError}</p>
        )}
      </div>
      <div>
        <label htmlFor="transaction-memo" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Memo (optional)</label>
        <div className="mt-1">
          <textarea
            id="transaction-memo" rows={3} value={memo}
            onChange={(e) => setMemo(e.target.value)} onBlur={() => handleBlur("memo")}
            disabled={isSubmitting}
            aria-invalid={touched.has("memo") && !!memoError}
            aria-describedby={memoError && touched.has("memo") ? "transaction-memo-error" : undefined}
            className={[
              "block w-full rounded-lg border px-3 py-2 text-sm shadow-sm transition-colors",
              "focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50",
        ...
        {touched.has("memo") && memoError && (
          <p id="transaction-memo-error" role="alert" className="mt-1 text-sm text-red-600 dark:text-red-400">{memoError}</p>
        )}
      </div>
      {errors.length > 0 && (
