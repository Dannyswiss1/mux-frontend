...
describe("WalletTable Responsive Layout", () => {
	...
	describe("mobile view (below lg breakpoint)", () => {
		...
		it("displays wallet information in card format", () => {
			...
			expect(screen.getAllByText("Last Activity").length).toBeGreaterThanOrEqual(1);
			
			// Metadata values
			expect(screen.getAllByText("1,250.50 XLM").length).toBeGreaterThanOrEqual(1);
		});
	...
	describe("responsive header", () => {
		...
		it("makes Add Wallet button full width on mobile", () => {
			const onAddWallet = vi.fn();
			const { container } = render(<WalletTable wallets={allWallets} onAddWallet={onAddWallet} />);
